...
const clsPrfm::stcData* clsPrfm::getDataPtr() const {
...
void clsPrfm::updateFrame() {
    ...
    this->m_f32InfiniGpFrame -= clsSingleton<clsPfSystem>::m_tpcSingleton->m_f32FrameRateAdd2;
    if (this->m_f32InfiniGpFrame > 0.0f) {
        return;
    }
    ...
    if (this->m_pcGearCtrl->m_eCtrlNo == CTRL_MAG) {
        this->m_pcGearCtrl->setUnuseGearLevel(LV_03);
    }
}
...
void clsPrfm::setup(clsCharacter* pcChara, clsGearCtrl* pcGearCtrl) {
    ...
    float fVar9 = rcWalk.f32Speed[1];
    fVar9 = rcWalk.f32Accele[2];
    float fVar8 = rcWalk.f32Accele[0];
    ...
    if (clsSingleton<clsGameMgr>::m_tpcSingleton->m_eMode == 3 || clsSingleton<clsGameMgr>::m_tpcSingleton->m_eMode == 9) {
        if (clsSingleton<clsGameMgr>::m_tpcSingleton->m_eMode == 3)
...
void clsPrfm::updateData() {
    ...
    float f32GpUsedRate = nspAgp::toaf32GpUsedRate[rcGearCtrl.m_pcPlayer->m_pcCharacter->m_eBaseType];
    ...
    this->m_sData.f32Grip =
        this->m_sBase.f32Grip * rcGearCtrl.m_sAddPrfmData.f32GripRate +
    ...
    this->m_sData.u32Ability =
        this->m_sBase.u32Ability | rcGearCtrl.m_sAddPrfmData.u32Ability;
    ...
    this->m_sData.f32GCtrlGpUseRate =
        this->m_sBase.f32GCtrlGpUseRate * f32GpUsedRate +
        rcGearCtrl.m_sAddPrfmData.f32GCtrlGpUseRate + 0.0f;
    this->m_sData.f32GDiveGpUseRate =
        this->m_sBase.f32GDiveGpUseRate * f32GpUsedRate +
    ...
    if (rcGearCtrl.m_pcPlayer->getPlayerType() != PLAYER_TYPE__GHOST) {
        if (this->m_sData.u32Ability & 0x20) {
            ...
            *(unsigned int*)((char*)rcGearCtrl.m_pcPlayer->m_pcPhysic->m_pcProxy->getShapePhantom() + 0x2c) =
                *(unsigned int*)((char*)rcGearCtrl.m_pcPlayer->m_pcPhysic->m_pcPhantom + 0x2c) &
