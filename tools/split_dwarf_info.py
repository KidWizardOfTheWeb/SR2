...
# Matches enum value lines: NAME = integer
...
re_global_decl = re.compile(r"^(?:[\w\s:<>*&]+?)\b\w+(?:\s*\[\d*\])*\s*;")
re_global_funcptr_decl = re.compile(r"^.+\(\*\s*\w+(?:\s*\[\d*\])*\s*\).+;$")
...
def _member_fingerprint(block: str) -> frozenset[str]:
    """
    Return a frozenset of member identifiers for deduplication.
    For structs: 'offset 0xNN, size 0xNN' annotations.
    For enums: 'NAME=value' tokens (enums have no offset/size annotations).
    """
    members = set(re_member.findall(block))
    for m in re_enum_value.finditer(block):
        members.add(f"{m.group(1)}={m.group(2)}")
...
def extract_globals_and_typedefs(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        if (
            re_typedef.match(code)
            or re_global_decl.match(code)
            or re_global_funcptr_decl.match(code)
        ):
